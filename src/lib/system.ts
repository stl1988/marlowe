...
- fal.ai is supported as an image generation provider (it is not OpenAI-compatible): when a fal.ai provider is configured, its main model path is used automatically, with automatic fallback to the configured fal.ai fallback model if the main one fails. fal.ai model paths contain multiple slashes (e.g. \`fal/openai/gpt-image-2.5/flare/text-to-image\`) and are valid image model IDs
